...
function checkNewRaceResults () {
	...
	request(options, function (error, rawResponse) {
		...
		let response = JSON.parse(rawResponse.body).MRData.RaceTable
		...
		if (response.season == dbData.cache.lastRace[0] && response["round"] == dbData.cache.lastRace[1]) {
			//DEBUG: This part is commented out so I can continue testing even when there is no new data
			//return;
		}
